    ...
    A: FrameAllocator,
{
    ...
    let mut new_table = {
        let frame = allocator.allocate_frame().expect("No frames left.");
    ...
    active_table.with(&mut new_table, &mut temporary_page, |mapper| {
        ...
        for section in elf_sections_tag.sections() {
            // TODO: mapper.identity() all pages in 'section'
            if !section.is_allocated() {
                // section is not load tot memory.
                continue;
            }

            assert!(
                section.start_address() % PAGE_SIZE == 0,
                "Sections must be aligned."
            );
